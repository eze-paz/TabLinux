...
// Ceiling on how much unacknowledged data we keep in flight, even when the peer
// advertises more. In-flight segments queue up host-side until the guest drains
// them, so an auto-tuned multi-megabyte Linux window would translate straight
// into host memory. 1 MiB over a ~67 ms round trip is ~15 MB/s of headroom.
const TCP_MAX_SEND_WINDOW = 1024 * 1024;
...
// Page-side DoH answer cache (bounded). The guest's resolver retries the same
// question on its own timeout, so a cached answer (with the ID patched to the
// current query) turns a repeat lookup into an instant local reply.
const fakeDnsCache = new Map();
const FAKE_DNS_CACHE_MAX = 256;
...
class GrowableRingbuffer
{
    ...
    /**
     * Like `peek`, but starting `offset` bytes past the front. A TCP sender needs
     * this to read data it has not sent yet while earlier, still-unacknowledged
     * bytes stay buffered for a possible retransmission.
     * @param {Uint8Array} dst_array
     * @param {number} offset
     */
    peek_at(dst_array, offset)
    {
        const length = Math.min(this.length - offset, dst_array.length);
        if(length <= 0) {
            return 0;
        }
        const buffer = this.buffer;
        const capacity = buffer.length;
        const start = (this.tail + offset) % capacity;
        const end = start + length;
        if(end > capacity) {
            const right = capacity - start;
            dst_array.set(buffer.subarray(start, capacity), 0);
            dst_array.set(buffer.subarray(0, length - right), right);
        }
        else {
            dst_array.set(buffer.subarray(start, end));
        }
        return length;
    }
...
function handle_fake_tcp(packet, adapter)
{
    ...
    if(packet.tcp.syn && !packet.tcp.ack) {
        const existing = adapter.tcp_conn[tuple];
        if(existing) {
            dbg_log("SYN to already opened port", LOG_FETCH);
            // Tear the old connection down, do not just forget it. Dropping the
            // table entry alone left the object alive and still wired to its
            // upstream stream, which keeps calling write() on it -- and its frames
            // carry the SAME four-tuple, so the guest accepts them as part of the
            // NEW connection while they come from a second, independent sequence
            // space (every connection starts at 1338). The peer's opening bytes
            // then arrive twice: ssh dies with "Bad packet length 1397966893",
            // which is ASCII "SSH-", a second banner in mid-stream. Reusing an
            // ephemeral port after a failed attempt is enough to hit this, which
            // is why the first connection to a new host was the one that broke.
            delete adapter.tcp_conn[tuple];
            existing.state = TCP_STATE_CLOSED;
            if(existing.on_close) {
                existing.on_close();
            }
        }
...
function deliverDohReply(adapter, packet, body)
{
    const reply = {
        eth: {
...
{
    const query = packet.udp.data;
    // Cache key = question section (skip the transaction ID, which differs per
    // attempt). On a hit, replay the stored answer with this query's ID — the
    // guest's resolver retries the same question and must see a matching ID.
    const qkey = query.length >= 12 ? query.subarray(12).join(",") : "";
    const cached = qkey && fakeDnsCache.get(qkey);
    if (cached) {
        const body = cached.slice();
        body[0] = query[0];
        body[1] = query[1];
        deliverDohReply(adapter, packet, body);
        return true;
    }
    ...
    // Retry once on failure: a transient error (cold TLS, preflight) used to
    // swallow the query with no reply at all, so the guest's first attempt
    // failed while its own retry succeeded — the "first ping always fails"
    // symptom.
    const attempt = (remaining) => {
        fetch(fetch_url, fetch_opts).then(async (resp) => {
            ...
            if (body.length && qkey) {
                fakeDnsCache.set(qkey, body);
                if (fakeDnsCache.size > FAKE_DNS_CACHE_MAX) {
                    fakeDnsCache.delete(fakeDnsCache.keys().next().value);
                }
            }
            deliverDohReply(adapter, packet, body);
        }).catch((err) => {
            if (remaining > 0) attempt(remaining - 1);
            else console.warn("[net] DoH query failed:", err);
        });
    };
    attempt(1);
    return true;
...
function parse_tcp(data, o) {
    ...
    // Window scale (RFC 7323, kind 3). Without it the peer's advertised window
    // cannot exceed the 16-bit field, which caps throughput at window/RTT
    // regardless of bandwidth: Linux saturates it at 65535 and a ~67 ms round
    // trip then pins the guest at ~1 MB/s. Only the SYN carries this.
    for(let i = TCP_HEADER_SIZE; i + 1 < offset && i + 1 < data.length;) {
        const kind = data[i];
        if(kind === 0) {
...
function write_tcp(spec, out) {
    ...
    if(tcp.options) {
        ...
        if(tcp.options.winscale !== undefined) {
            // Window scale (RFC 7323). Scaling applies only when BOTH sides send
            // this, so offering it in our SYN-ACK is what lets the peer advertise
            // a window past the 16-bit field. The shift here is OURS -- what our
            // own advertised windows should be multiplied by -- and 0 is correct:
            // this side reads from an in-memory buffer, so it needs no scaling.
            view.setUint8(doff, 0x03);
            view.setUint8(doff + 1, 0x03);
            view.setUint8(doff + 2, tcp.options.winscale);
            doff += 3;
        }
    }
...
export function TCPConnection(adapter)
{
    ...
    // Bytes sent but not yet acknowledged. They stay at the front of
    // `send_buffer` (removed only on ack), so this is also the offset of the next
    // byte to send, in the buffer and in the sequence space alike.
    this.in_flight = 0;
    // The peer's advertised receive window, which bounds `in_flight`. Learnt from
    // every incoming segment; 0 until the handshake supplies one.
    this.send_window = 0;
    // Left shift to apply to the peer's advertised window, from the window-scale
    // option in its SYN. 0 means scaling was not negotiated (windows as sent).
    this.snd_wscale = 0;
...
TCPConnection.prototype.accept = function(packet=undefined) {
    ...
    // The SYN carries the peer's initial window; seed the send window from it so
    // the first burst is not limited to a single segment. The window in a SYN is
    // never scaled, so this one is taken at face value.
    this.send_window = packet.tcp.winsize;
    // Accept window scaling only if the peer offered it, and only then answer
    // with our own option: scaling is in effect only when both SYNs carry it, so
    // shifting windows the peer never agreed to scale would overstate them by up
    // to 16384x -- and with no retransmission in this stack, overrunning the
    // peer's receive buffer is a hang, not a slowdown. The shift is clamped to
    // the 14 the RFC permits so it cannot exceed what a u16 shift can express.
    const options = { mss: (this.mtu - TCP_HEADER_SIZE - IPV4_HEADER_SIZE) };
    if(packet.tcp.winscale !== undefined) {
        this.snd_wscale = Math.min(packet.tcp.winscale, 14);
        options.winscale = 0;
    }
...
TCPConnection.prototype.process = function(packet) {
    ...
    // Track the peer's advertised receive window: it is what bounds how much
    // `pump` may keep in flight. Read from every segment, not just ones carrying
    // an ack, because a pure window update carries neither data nor new ack.
    // Post-handshake windows are scaled by the shift negotiated in the SYNs (0
    // when scaling was not agreed, leaving this exactly as before); the window in
    // a SYN itself is never scaled. Capped, because in-flight data queues up
    // host-side and an auto-tuned Linux window can reach megabytes -- 1 MiB over
    // a 67 ms round trip is already ~15 MB/s, far past what the link delivers.
    if(packet.tcp.winsize !== undefined) {
        const scaled = packet.tcp.syn
            ? packet.tcp.winsize
            : packet.tcp.winsize * Math.pow(2, this.snd_wscale | 0);
        this.send_window = Math.min(scaled, TCP_MAX_SEND_WINDOW);
    }
    ...
    if(this.last_received_ackn === undefined) {
        ...
        if(n_ack > 0) {
            ...
            this.seq += n_ack;
            // These bytes have left the window; the peer cannot acknowledge more
            // than we sent, but clamp rather than trust a bogus ack.
            this.in_flight = Math.max(0, this.in_flight - n_ack);
            this.pending = this.in_flight > 0;
            // Acks are what reopen the window, and nothing else pumps on this
            // path: without this, sending would stall until the next write().
            this.pump();
...
TCPConnection.prototype.pump = function() {
    // Keep the peer's whole advertised window in flight, not one segment at a
    // time. With a single segment outstanding, throughput is capped at
    // (segment size / round-trip time) no matter how fast the link is: in the
    // browser VM, where a round trip costs ~40 ms, that measured ~16 KB/s with
    // 680-byte segments. Linux advertises ~64 KB here — about 44 full-MSS
    // segments — so filling the window is worth two orders of magnitude.
    // A torn-down connection must never emit another frame. An upstream stream can
    // still hold a reference and keep feeding it, and its frames would carry a
    // four-tuple that now belongs to a different connection, injecting one peer's
    // bytes into another's sequence space.
    if(this.state === TCP_STATE_CLOSED) {
        return;
    }
    const mss = this.send_chunk_buf.length;
    // Honour a zero window: send NOTHING when the peer says its receive buffer is
    // full. It advertises that while it is busy draining -- apk writing a 4 MB
    // library to the emulated disk cannot keep up with the network -- and a
    // receiver DROPS segments that fall outside its window. With no
    // retransmission anywhere in this stack, one dropped segment stalls the
    // stream for good: the transfer dies and the guest reports a bare I/O error.
    //
    // Sending one segment anyway (which this did, to avoid a deadlock) was
    // survivable only while throughput was low enough that the peer's reader
    // always kept up. Liveness does not need it: the peer sends a window update
    // as an ack once its buffer drains, and acks call pump().
    let window = this.send_window | 0;
    // Diagnostic: shrink the window from the console (globalThis
    // .RISCV_TCP_WINDOW_CAP) to test whether throughput tracks it.
    const cap = globalThis.RISCV_TCP_WINDOW_CAP | 0;
    if(cap > 0) {
        window = Math.min(window, Math.max(cap, mss));
    }
    // Diagnostic: which side is the bottleneck. Stopping with the window full
    // means the peer's advertised window is the limit (window scaling would
    // help); stopping with nothing left to send means the data source upstream
    // is the limit (it would not). Off unless the page creates the object.
    const stats = globalThis.RISCV_TCP_STATS;
    let blocked_by_window = true;
    while(this.in_flight < window) {
        const unsent = this.send_buffer.length - this.in_flight;
        if(unsent <= 0) {
            blocked_by_window = false;
            break;
        }
        const n = Math.min(unsent, mss, window - this.in_flight);
        const data = this.send_chunk_buf.subarray(0, n);
        if(this.send_buffer.peek_at(data, this.in_flight) !== n) {
            blocked_by_window = false;
            break;
        }
        const reply = this.ipv4_reply();
        reply.tcp.psh = true;
        // ipv4_reply() stamps the first unacknowledged byte. Every segment after
        // the first must carry its own offset, or they all claim one sequence
        // number and the peer reassembles a corrupt stream.
        reply.tcp.seq = (this.seq + this.in_flight) >>> 0;
        reply.tcp_data = data;
        this.net.receive(make_packet(this.net.eth_encoder_buf, reply));
        this.in_flight += n;
        if(stats) {
            stats.segments++;
            stats.bytes += n;
            if(this.in_flight > stats.max_in_flight) {
                stats.max_in_flight = this.in_flight;
            }
        }
    }
    if(stats) {
        // Counted separately from a merely-full window: this is the peer telling
        // us to stop entirely, which is what used to cost a dropped segment.
        if(window <= 0 && this.send_buffer.length > this.in_flight) {
            stats.stopped_zero_window = (stats.stopped_zero_window | 0) + 1;
        }
        if(blocked_by_window) {
            stats.stopped_window_full++;
        }
        else {
            stats.stopped_buffer_empty++;
        }
        stats.last_window = window;
    }
    // The close path delays its FIN while anything is still outstanding.
    this.pending = this.in_flight > 0;
};
